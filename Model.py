...
class Model(nn.Module):
    def __init__(self, image_embedding, text_embedding, audio_embedding=None):
        ...
        if args.model_type == 'spectral_diffmm':
            num_modalities = 3 if audio_embedding is not None else 2
            self.gcnLayers = nn.Sequential(*[GHSNO(args.latdim, args.latdim, num_modalities) for i in range(args.gnn_layer)])
        else:
    ...
    def forward_MM(self, adj, image_adj, text_adj, audio_adj=None):
        ...
        if args.model_type == 'spectral_diffmm':
            if audio_adj == None:
                embedsLst = [embedsImage, embedsText]
            else:
                embedsLst = [embedsImage, embedsText, embedsAudio]
            for gcn in self.gcnLayers:
                embedsModal = gcn(adj, embedsLst)
                embedsLst = [embedsModal] * len(embedsLst) # Simple fallback if multiple layers
            embeds = embedsModal
        else:
            embedsLst = [embedsModal]
            for gcn in self.gcnLayers:
