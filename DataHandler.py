...
class DataHandler:
    def __init__(self, base_path=None):
        """
        base_path: đường dẫn tới thư mục Datasets.
            - Mặc định: None → dùng args.dataset_path (từ CLI hoặc config dict)
            - Trên Colab: truyền vào '/content/drive/MyDrive/DiffMM_Data'
        """
        if base_path is not None:
            predir_root = base_path
        else:
            predir_root = getattr(args, 'dataset_path', './Datasets')

        if args.data == 'baby':
            predir = os.path.join(predir_root, 'baby') + '/'
        elif args.data == 'sports':
            predir = os.path.join(predir_root, 'sports') + '/'
        elif args.data == 'tiktok':
            predir = os.path.join(predir_root, 'tiktok') + '/'
        else:
            raise ValueError(f"Dataset '{args.data}' không được hỗ trợ. Chọn: baby / sports / tiktok")
    ...
    def LoadData(self):
        ...
        if args.model_type == 'spectral_diffmm':
            ...
            self.spectral_bands = SpectralUtils.spectral_band_split(full_signal, self.torchLaplacian, args.cheby_k, args.num_bands).cpu()
            self.diffusionData = DiffusionData(self.spectral_bands)
        else:
