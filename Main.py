...
from SpectralDiffusion import SpectralDiffusion
from SpectralBandMasking import SpectralBandMasking
import numpy as np
...
class Coach:
    ...
    def save_checkpoint(self, epoch):
        ...
        if args.model_type == 'flowmatching_optimized':
            ...
            if args.model_type == 'spectral_diffmm':
                checkpoint['band_masker'] = self.band_masker.state_dict()
    ...
    def load_checkpoint(self, path):
        ...
        if args.model_type == 'flowmatching_optimized':
            ...
            if args.model_type == 'spectral_diffmm' and 'band_masker' in checkpoint:
                self.band_masker.load_state_dict(checkpoint['band_masker'])
    ...
    def prepareModel(self):
        ...
        if args.model_type == 'flowmatching_optimized':
            ...
            if args.model_type == 'spectral_diffmm':
                self.diffusion_model = SpectralDiffusion(args.noise_scale, args.noise_min, args.noise_max, args.steps, args.num_bands, args.spectral_denoise_bands).to(device)
                self.band_masker = SpectralBandMasking(args.band_mask_ratio).to(device)
            elif args.model_type == 'flowmatching_original':
    ...
    def trainEpoch(self):
        ...
        for i, batch in enumerate(diffusionLoader):
            ...
            if args.model_type == 'flowmatching_optimized':
                ...
                if args.model_type == 'spectral_diffmm':
                    batch_item_masked = self.band_masker.mask_bands(batch_item)
                    diff_loss_image, gc_loss_image = self.diffusion_model.training_losses(self.denoise_model_image, batch_item_masked, iEmbeds, batch_index, image_feats)
                    diff_loss_text, gc_loss_text = self.diffusion_model.training_losses(self.denoise_model_text, batch_item_masked, iEmbeds, batch_index, text_feats)
                    if args.data == 'tiktok':
                        diff_loss_audio, gc_loss_audio = self.diffusion_model.training_losses(self.denoise_model_audio, batch_item_masked, iEmbeds, batch_index, audio_feats)
                else:
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(diffusionLoader):
                ...
                elif args.model_type == 'spectral_diffmm':
                    x_start_bands = torch.randn_like(batch_item) # batch_item is (batch, num_bands, item)
                    denoised_batch_image = self.diffusion_model.p_sample_spectral(self.denoise_model_image, x_start_bands, args.sampling_steps)
                    denoised_batch_text = self.diffusion_model.p_sample_spectral(self.denoise_model_text, x_start_bands, args.sampling_steps)
                    if args.data == 'tiktok':
                        denoised_batch_audio = self.diffusion_model.p_sample_spectral(self.denoise_model_audio, x_start_bands, args.sampling_steps)
                else:
